...
from restoration_adaptation.ram.models.ram_lora import ram
from restoration_adaptation.ram import inference_ram as inference
from torchvision import transforms
from restoration_adaptation.ras import Generator_with_pretrain as Generator

logger = logging.getLogger(__name__)

class FeatureProjection(Backbone):
    ...
    def __init__(
        ...
        self.feature_dims = cfg.MODEL.FEATURE_PROJECTION.FEATURE_DIMS
        self.grouped_indices = cfg.MODEL.FEATURE_PROJECTION.GROUPED_INDICES
        self.feature_strides = cfg.MODEL.FEATURE_PROJECTION.FEATURE_STRIDES
        self.out_features = cfg.MODEL.FEATURE_PROJECTION.OUT_FEATURES
        ...
        for indices in self._sorted_grouped_indices:
            ...
            name = f"res{int(math.log2(stride))}"
        ...
        self.cfg = cfg.MODEL.RASS

        # build restoration model
        net_res = Generator(self.cfg)
        net_res.set_train()
        if self.cfg.enable_xformers_memory_efficient_attention:
            if is_xformers_available():
                net_res.unet.enable_xformers_memory_efficient_attention()
            else:
                raise ValueError("xformers is not available, please install it by running `pip install xformers`") 
        if self.cfg.gradient_checkpointing:
            net_res.unet.enable_gradient_checkpointing()
        net_res.unet.set_adapter(['default_encoder_s2', 'default_decoder_s2', 'default_others_s2'])
        self.sd = net_res.to('cuda')
        ...
        RAM = ram(pretrained=self.cfg.ram_path,
        ...
        DAPE = ram(pretrained=self.cfg.ram_path,
                pretrained_condition=self.cfg.ram_ft_path,
                image_size=384,
                vit='swin_l')
        DAPE.eval()
        DAPE.requires_grad_(False)
        DAPE.to("cuda", dtype=torch.float16)

        self.ram = RAM
        self.dape = DAPE

        self.pos_prompt_vsd = cfg.MODEL.RASS.pos_prompt_vsd
    ...
    def single_forward(self, img, batch):
        # get text prompts
        ram = self.ram
        ram_transforms = self.ram_transforms
        x_tgt = batch["output_pixel_values"].to('cuda')
        x_ram = ram_transforms(x_tgt*0.5+0.5)
        caption = inference(x_ram.to(dtype=torch.float16), ram)
        batch["prompt"] = [f'{each_caption}, {self.pos_prompt_vsd}' for each_caption in caption]

        # save memory
        input_image_size = img.shape[-2:]
        sd = self.sd
        features = sd(img, batch=batch)
    ...
    def slide_forward(self, batch):
        # get text prompts
        if self.cfg.use_degradation:
            ram = self.dape
        else:
            ram = self.ram
        ram_transforms = self.ram_transforms

        img = batch["conditioning_pixel_values"].to('cuda')
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                x_ram = ram_transforms(crop_img*0.5+0.5)
                caption = inference(x_ram.to(dtype=torch.float16), ram)
                batch["prompt"] = [f'{each_caption}, {self.pos_prompt_vsd}' for each_caption in caption]
                crop_features = self.single_forward(crop_img, batch)
    ...
    def forward(self, batch):
        ...
        if (self.training and not self._slide_training) or not self._slide_inference:
            return self.single_forward(img, batch)
        else:
            return self.slide_forward(batch)
